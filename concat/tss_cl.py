...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
log_path = os.path.join(log_dir, 'concat_tss_cl.log')
...
class IG_RGCN(nn.Module):
    def __init__(self, embed_table, in_feats, hid_feats, rel_names, agg_fn):
        super().__init__()
        
        self.sequence_embed = embed_table
        sequence_embed_size = embed_table.shape[-1]
        self.embed = nn.Linear(in_feats, hid_feats)
        self.conv1 = dgl.nn.pytorch.hetero.HeteroGraphConv({
            rel: IGConv(hid_feats+sequence_embed_size, hid_feats)
    ...
    def forward(self, blocks, x, encode=False):
        ...
        user_sequence_feature = self.sequence_embed[blocks[0].srcdata[dgl.NID]]
        x['user'] = torch.cat([user_original_feature, user_sequence_feature], dim=1)
        h = self.conv1(blocks[0], x)
        h = {k: F.relu(self.attn(v), inplace=True) for k, v in h.items()}

        user_sequence_feature = self.sequence_embed[blocks[1].srcdata[dgl.NID]]
        user_original_feature = h['user']
        h['user'] = torch.cat([user_original_feature, user_sequence_feature], dim=1)      
...
def run():
    ...
    embed_table = torch.load("/home/zqxu/MHTGNN/data/embedding/tfencoder_Xinye_tss_Npair_embed.pt")
    model = IG_RGCN(embed_table, n_hetero_features, args.hid_dim, hetero_graph[1].etypes, args.agg_fn)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if results[0] > bestauc:
                bestauc = results[0]
                bestepoch = epoch
                torch.save(model.state_dict(), "/home/zqxu/MHTGNN/model_save/IHG_tss_cl_prarams.pth")
...
def generate_embed():
    ...
    embed_table = torch.load("/home/zqxu/MHTGNN/data/embedding/tfencoder_Xinye_tss_Npair_embed.pt")
    model = IG_RGCN(embed_table, n_hetero_features, args.hid_dim, hetero_graph[1].etypes, args.agg_fn)
    model.load_state_dict(torch.load("/home/zqxu/MHTGNN/model_save/IHG_tss_cl_prarams.pth"))
    num_nodes = hetero_graph[1].num_nodes('user')
    embed_table = torch.zeros(num_nodes, 1)

    sampler = dgl.dataloading.MultiLayerFullNeighborSampler(2)
    dataloader_all = dgl.dataloading.NodeDataLoader(
        hetero_graph[1], 
        {'user': [i for i in range(num_nodes)]}, 
        sampler, 
    ...
    model.eval()
    with torch.no_grad():
        for input_nodes, output_nodes, blocks in tqdm(dataloader_all):
            user_feature = blocks[0].nodes['user'].data['feature']
            node_features = {'user': user_feature}
            h = model(blocks, node_features)
            embed_table[output_nodes['user'].long()] = h
    torch.save(embed_table, "/home/zqxu/MHTGNN/data/embedding/IHG_Xinye_tss_cl_logits.pt")
