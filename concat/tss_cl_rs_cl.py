...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
log_path = os.path.join(log_dir, 'concat_tss_cl_rs_cl.log')
...
def embed_vis():
    ...
    rs_embed = torch.load("/home/zqxu/MHTGNN/data/embedding/tfencoder_Xinye_rs_Npair_embed.pt")
    model = IG_RGCN(tss_embed, rs_embed, n_hetero_features, args.hid_dim, hetero_graph[1].etypes, args.agg_fn)
    model.load_state_dict(torch.load("/home/zqxu/MHTGNN/model_save/IHG_tss_cl_rs_cl_0.8332_prarams.pth"))
    criterion = nn.BCELoss(reduction='sum')
    ...
    with torch.no_grad():
        for input_nodes, output_nodes, blocks in tqdm(dataloader_test):
            user_feature = blocks[0].nodes['user'].data['feature']
            node_features = {'user': user_feature}
            logits = model(blocks, node_features, label[output_nodes['user']])
            prob = logits
            pred = torch.where(prob > 0.5, 1, 0)
            test_label = label[output_nodes['user'].long()].reshape(-1, 1)
            loss = criterion(prob, test_label)
        ...
        results = np.array(results)
        results = np.mean(results, axis=0)
        log.record("Test Loss: %.5f, AUC: %.4f, KS: %.4f, PRE: %.4f, RECALL: %.4f F1:%.4f, ACC: %.4f" % (
            test_loss, results[0], results[1], results[2], results[3], results[4], results[5])
        )
